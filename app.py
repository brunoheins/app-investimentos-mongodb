...
def tela_acesso():
    ...
    # --- A MÁGICA CONTRA A TELA FANTASMA (GHOSTING) ---
    # Se o login foi validado, fazemos uma rodada "vazia" instantânea.
    # O Streamlit desenha uma tela em branco (apagando o login nativamente)
    # e logo em seguida carrega a tela pesada de Resumo. Zero erros, zero ghosting!
    if st.session_state.get('pre_login_sucesso'):
        st.session_state.pre_login_sucesso = False
    ...
    with col2:
        ...
        with tab_login:
            # 1. Formulário nativo (Garante o Enter perfeito, sem bugar ao clicar fora)
            with st.form("form_login"):
                email_input = st.text_input("E-mail")
                senha_input = st.text_input("Senha", type="password")
                submit_login = st.form_submit_button("Entrar", use_container_width=True, type="primary")
            
            # 2. Lógica fora do formulário (Garante zero erros vermelhos)
            if submit_login:
                agora = time.time()
                if agora - st.session_state.last_login_time < 3:
                    ...
                    if usuario:
                        ...
                        if status_usuario == 'Ativo':
                            # Prepara a sessão e limpa cache
                            ...
                            # Aciona o gatilho da limpeza visual (Flush) e recarrega
                            st.session_state.pre_login_sucesso = True
                            st.rerun() 
