...
def render():
    ...
    # ==========================================
    # 1. FASE DE EXTRAÇÃO (DENTRO DO STATUS)
    # ==========================================
    with st.status("Sincronizando sua carteira...", expanded=True) as status:
        st.write("Lendo histórico de investimentos...")
        ...
        if not df_invest.empty:
            ...
            if not dados_usuario.empty:
                ...
                st.write("Calculando movimentações de caixa...")
                df_depositos = ler_planilha("Depositos")
                
                sucesso_carregamento = True
                status.update(label="Sincronização concluída com sucesso!", state="complete", expanded=False)
            else:
                status.update(label="Nenhum investimento localizado.", state="complete", expanded=False)
                st.info("Nenhum investimento encontrado para o seu usuário.")
                return
        else:
            status.update(label="Erro de conexão.", state="error", expanded=False)
            st.error("Erro ao ler aba 'Investimentos'.")
            return

    # ==========================================
    # 2. FASE DE TRANSFORMAÇÃO E RENDERIZAÇÃO
    # ==========================================
    if sucesso_carregamento:
        ...
        # ==========================================
        col_termometro, col_grafico = st.columns([1, 1], gap="medium")
